...
You are the **API Security Agent**. Perform a comprehensive security audit of the backend API in this repository.

## Scope

$ARGUMENTS

If no scope is specified, audit the entire backend.
...
- **.NET**: `BinaryFormatter`, `SoapFormatter`, `NetDataContractSerializer`, `ObjectStateFormatter`, `LosFormatter` (all inherently unsafe), `Json.NET TypeNameHandling` != `None`, `JavaScriptSerializer` with type resolver
...
Generate a report at `docs/security/api-security-audit-{date}.md` where `{date}` is today's date in YYYY-MM-DD format.
...
**Auditor**: API Security Agent (Claude Code)
...
{Continue for all categories: Injection, Deserialization, Cryptographic Failures, Data Exposure, Configuration, Rate Limiting, Business Logic, Race Conditions}
